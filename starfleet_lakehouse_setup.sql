-- Starfleet Data Platform: fictional Iceberg lakehouse for MV optimization examples.
-- Flow: raw (one nested golden-record document per vessel) > transform MVs (slice each sub-object out) > analytics MVs (dedup and flatten).
-- Transform tables are MVs here so a refresh visibly begets the next layer. In production they would be plain tables. Engine: Trino / Starburst Galaxy. Author: Charlton Julius.
-- Run top to bottom once. Refresh order matters: transform before intermediate before analytics. A commented teardown sits at the end.
-- In-universe content is Star Trek prior to 2008 (TOS, TAS, films I-X, TNG, DS9, VOY, ENT).
-- ============================================================
-- SCHEMAS
-- ============================================================
...
-- vessel_delete_signal is a separate tombstone feed (deletes arrive on their own stream and feed the intermediate obituary layer, not transform).
-- ============================================================
...
-- Modeled as MVs so a refresh visibly produces this layer from raw. No dedup here, that happens in analytics.
-- ============================================================
...
-- ============================================================
-- INTERMEDIATE LAYER: tombstone (obituary) MV. Latest delete per entity.
-- ============================================================
...
FROM flat;
-- ============================================================
-- REFRESH: transform first, then intermediate, then analytics. Each layer begets the next.
-- ============================================================
